...
const { getAllFilePath, getFileMatter, isMatterDataEmpty, grayMatterWriter } = require('./api');
...
function formatMarkdown() {
  getAllFilePath().map((path) => {
    ...
    const matterEmpty = isMatterDataEmpty(matterData);

    var createTime = data.createTime || Date.now();
    ...
    var createTimeStr = dayjs(createTime).format('YYYY年MM月DD日');
