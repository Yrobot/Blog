...
const renderer = {
  ...
  code(code, language) {
    // 检测是否为 mermaid 代码块
    if (language === "mermaid") {
      // 将 mermaid 代码包装在特殊的 div 中，以便客户端渲染
      return `<pre class="mermaid">${code}</pre>`;
    }
    // 其他代码块使用默认的高亮处理
    const highlighted = require("highlight.js").highlightAuto(
      code,
...
const catalogToStr = (catalog) => {
  return catalog
    .map((item) => `${[...Array(item?.level ?? 0)].join("  ")}- ${item.text}`)
    .join("\n");
};
