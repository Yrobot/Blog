...
#### 理想情况：auth 系统更新登陆方式或关闭某个登陆方式时，业务无感

下面我们来进一步优化一下实现方案

业务和 auth 系统接管的界限是 iframe，

**所以我们只需要把 auth 逻辑相关的放到 iframe 内处理，和业务相关的放到 iframe 外。**

以此为标准设计 auth client 组件

auth 的 client 组件做一些便于使用的简单映射逻辑即可，比如登陆、登出、获取 token 等逻辑的封装，个性化登陆组件的逻辑等。
