...
1. **需要外部数据？** → MCP
2. **需要自动触发？** → Hook
3. **需要角色切换？** → Output Style
4. **需要快速指令？** → Slash Command
5. **需要复杂流程？** → Subagent
6. **需要单一能力？** → Skill
7. **需要打包分发？** → Plugin
...
- **MCP** = 外部 API（数据源）
- **Hook** = 自动化脚本（事件响应）
- **Slash Command** = 快捷键（提示词别名）
- **Output Style** = 角色切换（行为模式）
- **Plugin** = 安装包（组件集合）

### 关键收获

1. **Skills 不是函数**：无法显式调用，只能语义触发
2. **Subagent 可以编排 Skills**：提示词中明确指示"使用 XX skill"
3. **结构化输出需要提前定义**：在提示词中明确 JSON Schema
4. **不要为了用而用**：每个工具都有适合的场景，选择时基于"因为...所以..."的逻辑
